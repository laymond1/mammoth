...
class MobileNetV2(MammothBackbone):
    def __init__(
        self,
        ...
        self.features_layers = nn.Sequential(*features)
    ...
    def forward(self, x: Tensor, returnt='out') -> Tensor:
        return self._forward_impl(x, returnt)


@register_backbone("mobilenet_v2")
def mobilenet_v2(num_classes: int) -> MobileNetV2:
    """MobileNetV2 model architecture from the
...
@register_backbone("mobilenet_v2_pt")
def mobilenet_v2(num_classes: int) -> MobileNetV2:
    """MobileNetV2 model architecture from the
    `"MobileNetV2: Inverted Residuals and Linear Bottlenecks" <https://arxiv.org/abs/1801.04381>`_ paper.
    ...
    pretrain_weights = MobileNet_V2_Weights.DEFAULT
