...
def parse_args():
    ...
    from utils.conf import warn_once
    from utils.args import add_initial_args, add_management_args, add_experiment_args, add_online_learning_args, add_configuration_args, clean_dynamic_args, \
...
def extend_args(args, dataset):
    ...
    if args.validation:
        if args.validation_mode == 'current':
            assert dataset.SETTING in ['online-il', 'class-il', 'task-il'], "`current` validation modes is only supported for class-il and task-il settings (requires a task division)."
...
def main(args=None):
    ...
    from utils.online_training import train as online_train
    ...
    print(f"Set a Prompt-based CL method ({args.model})")
