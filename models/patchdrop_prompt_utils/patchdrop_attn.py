        ...
        def forward(self, x, register_blk=-1, prompt=None, q=None, train=False, feat=False) -> torch.Tensor:
            if prompt is not None:
                self.patchdrop = PatchDropout(
                    keep_rate=self.prompt_keep_rate, 
                    sampling=self.sampling, 
                    token_shuffling=self.token_shuffling
                )
            else:
                self.patchdrop = PatchDropout(
                    keep_rate=self.query_keep_rate, 
                    sampling=self.sampling, 
                    token_shuffling=self.token_shuffling
                )
...
def apply_patch(
    ...
    model.prompt_keep_rate = 0.0
    model.query_keep_rate = 0.0
    ...
    model._patchdrop_info = {
        "prompt_keep_rate": model.prompt_keep_rate,
        "query_keep_rate": model.query_keep_rate,
        "size": None,
        "source": None,
