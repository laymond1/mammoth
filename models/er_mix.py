...
class ErMix(ContinualModel):
    NAME = 'er_mix'
    COMPATIBILITY = ['class-il', 'domain-il', 'task-il', 'general-continual']

    def __init__(self, backbone, loss, args, transform):
        super(ErMix, self).__init__(backbone, loss, args, transform)
    ...
    def process_inc(self, inputs, labels, not_aug_inputs):
        pred = self.net(inputs)
        loss = self.loss(pred, labels)

        self.inc_classes = labels.unique()
        # self.seen_so_far = list(set(self.seen_so_far + self.inc_classes.tolist()))

        return loss

    def process_re(self, inputs, labels, not_aug_inputs):
        pred = self.net(inputs)
        loss = self.loss(pred, labels)
    ...
    def observe(self, inputs, labels, not_aug_inputs):
        ...
        # mixup loss
        mixed_input, labels_a, labels_b, lambda_ = MixUp(inputs, labels, alpha=self.args.gamma)
