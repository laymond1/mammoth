...
def _cfg(url='', **kwargs):
    # print( 'Poolformer mean', IMAGENET_DEFAULT_MEAN, 'std', IMAGENET_DEFAULT_STD)
    return {
        'url': url,
...
class PoolFormer(nn.Module):
    ...
    def forward(self, x, prompt=None, prompt_name=None, q=None, train=False, task_id=None):
        #(self, x, register_blk=-1, prompt=None, prompt_name=None, q=None, train=False, task_id=None):
        # input embedding
        ...
        #
        # i = 0
        # for idx, block_block in enumerate(self.network):
        #     if isinstance(block_block, nn.Sequential):
        #         for block in block_block:
        #             if prompt is not None:
        #                 if train:
        #                     _, loss, x = prompt.forward(q, i, x, train=True, task_id=task_id)
        #                     prompt_loss += loss
        #                 else:
        #                     _, _, x = prompt.forward(q, i, x, train=False, task_id=task_id)
        #             else:
        #                 p_list = None
        #             x = block(x)
        #             i += 1
