...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, register_blk=-1, prompt=None, q=None, train=False, drop_rate=0.0, sparse_type='random'):
        B = x.shape[0]
        ...
        # 1) input patch pruning
        if q is not None:
            patch = q[:, 1:, :]
            q = q[:, 0, :]
            if drop_rate > 0.0 and train:
                with torch.no_grad():
                    if sparse_type == 'random':
                        ...
                        x = self.topk_l2_token_masking(x, patch, drop_rate=drop_rate)
                    else:
                        raise ValueError('Unknown pruning type: {}'.format(sparse_type))
    ...
    def topk_l2_token_masking(self, x, patch, drop_rate=0.0):
        ...
        # with torch.no_grad():
        # out, _ = model.feat(images)
        # out = out[:, 1:]  # (B, N, D)
        
        # L2 norm 계산: (B, N)
        norms = torch.norm(patch, dim=2)
        
        # Top-k index (가장 정보량 높은 토큰 유지)
        k = int((1 - drop_rate) * N)
        topk_idx = torch.topk(norms, k=k, dim=1).indices  # (B, k)
