...
def prune_parse_arguments(parser):
    ...
    parser.add_argument('--sp-prune-before-retrain', type=binary_to_boolean_type, default=True,
                        help="Prune the loaded model before retrain, in case of loading a dense model")
    parser.add_argument('--output-compressed-format', type=binary_to_boolean_type, default=False,
                        help="output compressed format")
    parser.add_argument("--sp-grad-update", type=binary_to_boolean_type, default=False,
    ...
    parser.add_argument("--sp-grad-restore-threshold", type=float, default=-1,
                        help="When the decay")
    parser.add_argument("--sp-global-magnitude", type=binary_to_boolean_type, default=False,
        ...
        with torch.no_grad():
            for name, W in (self.model.named_parameters()):
                if name in self.masks:
                    ...
                    mask = self.masks[name].to(W.device)
                    W.mul_((mask != 0).type(dtype))
        ...
        with torch.no_grad():
            for name, W in (self.model.named_parameters()):
                if name in self.gradient_masks:
                    dtype = W.dtype
                    # Ensure the mask is on the same device as the weights
                    mask = self.gradient_masks[name].to(W.device)
                    (W.grad).mul_((mask != 0).type(dtype))
