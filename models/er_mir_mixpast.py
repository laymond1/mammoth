...
def get_parser() -> ArgumentParser:
    ...
    parser.add_argument('--subsample', type=int, default=50,
                        help='Number of samples to use for MIR subsampling.')
...
class ErMIRMixPast(ContinualModel):
    NAME = 'er_mir_mixpast'
    COMPATIBILITY = ['class-il', 'domain-il', 'task-il', 'general-continual']

    def __init__(self, backbone, loss, args, transform):
        super(ErMIRMixPast, self).__init__(backbone, loss, args, transform)
        self.buffer = BufferRetrieve(args, self.args.buffer_size, self.device, self.net, retrieve='mir')
        self.task = 0
