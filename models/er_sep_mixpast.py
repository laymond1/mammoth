...
class ErSepMixPast(ContinualModel):
    NAME = 'er_sep_mixpast'
    COMPATIBILITY = ['class-il', 'domain-il', 'task-il', 'general-continual']

    def __init__(self, backbone, loss, args, transform):
        super(ErSepMixPast, self).__init__(backbone, loss, args, transform)
    ...
    def observe(self, inputs, labels, not_aug_inputs):
        ...
        if not self.buffer.is_empty():
            ...
            if self.task > 0:
                index = ~torch.isin(buf_labels, self.inc_classes) # self.inc_classes 제거
                # mixup loss
