...
Note:
    Dynamic-Prompt USES A CUSTOM BACKBONE: ``.
    The backbone is a ViT-B/16 pretrained on ImageNet 1k.
"""
...
from models.superprompt_utils.model import PromptModel
...
class DynamicPrompt(ContinualModel):
    ...
    def get_parser(parser) -> ArgumentParser:
        # Parameters
        parser.add_argument('--config_file', type=str, default='models/superprompt_utils/final_3min_space_eval.yaml', help='path to config file')
        parser.add_argument('--elastic_type', type=str, default='elastic_T1', help='ElasticViT type in search space yaml file')
        parser.add_argument('--e_prompt_layer_idx', type=int, default=[2, 3, 4, 5], nargs="+", help='the layer index of the E-Prompt')
        ...
        # Dynamic Prompt Specific
        parser.add_argument('--change_arch_at_task', type=int, default=-1, help='the task to change the architecture')
        parser.add_argument('--change_arch_to', type=str, default='elastic_S1', help='the architecture to change to')
    ...
    def __init__(self, backbone, loss, args, transform, dataset=None):
        ...
        # set ElasticViT model
        elastic_arch = getattr(self.net.config, self.args.elastic_type)
        self.net.feat.set_arch(*elastic_arch)
    
    def begin_task(self, dataset):
        if self.current_task+1 == self.args.change_arch_at_task:
            print(f"Change architecture at task {self.current_task}")
            # change to the smallest architecture
            elastic_arch = getattr(self.net.config, self.args.change_arch_to)
            self.net.feat.set_arch(*elastic_arch)
            print("Changed Architecture:", str(self.args.change_arch_to))
            print(f"Change to the smallest architecture: {elastic_arch}")
    ...
    def get_parameters(self):
        return [p for n, p in self.net.named_parameters() if n.startswith("prompt") or n.startswith("head")]
