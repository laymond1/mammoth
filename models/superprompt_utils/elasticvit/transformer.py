...
from models.superprompt_utils.elasticvit.common_ops import LNSuper, BNSuper1d, BNSuper2d, drop_connection
from models.superprompt_utils.elasticvit.cnn import MobileBlock
from models.superprompt_utils.elasticvit.sample_utils import mutate_dims
...
class SuperAttention(nn.Module):
    ...
    def _attention(self, x: torch.Tensor, prompt: torch.Tensor = None):
        B, HW, C = x.shape
        ...
        q = q.permute(0, 2, 1, 3) # [B, 3, 64, 16]
        k = k.permute(0, 2, 1, 3) # [B, 3, 64, 16]
        ...
        v = v.permute(0, 2, 1, 3) # [B, 3, 64, 32]

        if prompt is not None:
            pk, pv = prompt
            pk = pk.reshape(B, -1, self.sampled_num_heads, self.sampled_qkdim).permute(0, 2, 1, 3)
            pv = pv.reshape(B, -1, self.sampled_num_heads, self.sampled_vdim).permute(0, 2, 1, 3)
        ...
        if prompt is not None:
            attn[:, :, :, pk.shape[2]:] += relative_position_bias
        else:
    ...
    def forward(self, x, prompt=None):
        ...
        out = self._attention(x, prompt=prompt)
