...
from models.atom_prompt_utils.merge import bipartite_soft_matching, merge_source, merge_wavg
from models.atom_prompt_utils.utils import AToM_parse_r


class AToMBlock(Block):
    """
    Modifications:
     - Apply Adaptive Token Merging (AToM) between the attention and mlp blocks
    ...
    def forward(self, x: torch.Tensor, register_hook: bool = False, prompt: torch.Tensor = None, sparse: bool = True) -> torch.Tensor:
        ...
        attn_size = self._atom_info["size"] if self._atom_info["prop_attn"] else None
        # Full Token Forward
        if not sparse:
        ...
        # Sparse Token Forward
            ...
            r = self._atom_info["r"].pop(0)
            if r > 0:
                # Apply AToM here
                merge, _ = bipartite_soft_matching(
                    metric,
                    r,
                    self._atom_info["class_token"],
                    self._atom_info["distill_token"],
                )
                if self._atom_info["trace_source"]:
                    self._atom_info["source"] = merge_source(
                        merge, x, self._atom_info["source"]
                    )
                x, self._atom_info["size"] = merge_wavg(merge, x, self._atom_info["size"])

            x = x + self._drop_path2(self.mlp(self.norm2(x)))
            return x


class AToMAttention(Attention):
    """
    Modifications:
...
def make_atom_class(transformer_class):
    class AToMVisionTransformer(transformer_class):
        """
        Modifications:
        ...
        def forward(self, x, register_blk=-1, prompt=None, q=None, train=False, feat=False) -> torch.Tensor:
            if prompt is not None:
                self._atom_info["r"] = AToM_parse_r(len(self.blocks), self.prompt_r)
            else:
                self._atom_info["r"] = AToM_parse_r(len(self.blocks), self.query_r)
            self._atom_info["size"] = None
            self._atom_info["source"] = None
...
def apply_patch(
    ...
    """
    Applies AToM to this transformer. Afterward, set r using model.r.

    If you want to know the source of each token (e.g., for visualization), set trace_source = true.
    The sources will be available at model._atom_info["source"] afterward.

    For proportional attention, set prop_attn to True. This is only necessary when evaluating models off
    the shelf. For trianing and for evaluating MAE models off the self set this to be False.
    """
    AToMVisionTransformer = make_atom_class(model.__class__)

    model.__class__ = AToMVisionTransformer
    model.prompt_r = 0
    model.query_r = 0
    ...
    if hasattr(model, "dist_token") and model.dist_token is not None:
        model._atom_info["distill_token"] = True

    for module in model.modules():
        if isinstance(module, Block):
            module.__class__ = AToMBlock
            module._atom_info = model._atom_info
        elif isinstance(module, Attention):
            module.__class__ = AToMAttention
