...
# CIL CONFIG
NAME="Online BFPrompt with module2"
NOTE="si-blurry-bfprompt-module2"  # Short description of the experiment.
...
for DATASET in "${DATASETS[@]}"; do
    ...
    if [ "$DATASET" == "seq-cifar100-224" ]; then
        MEM_SIZE=0 ONLINE_ITER=3 EVAL_PERIOD=1000 F_EVAL_PERIOD=10000
        BATCHSIZE=64; LR=5e-4 OPT_NAME="adam" SCHED_NAME="default"
        N_TASKS=5

    elif [ "$DATASET" == "seq-tinyimg-224" ]; then
        MEM_SIZE=0 ONLINE_ITER=3 EVAL_PERIOD=2000 F_EVAL_PERIOD=20000
        BATCHSIZE=64; LR=5e-4 OPT_NAME="adam" SCHED_NAME="default"
        N_TASKS=5

    elif [ "$DATASET" == "seq-imagenet-r" ]; then
        MEM_SIZE=0 ONLINE_ITER=3 EVAL_PERIOD=1000 F_EVAL_PERIOD=6000
        BATCHSIZE=64; LR=5e-4 OPT_NAME="adam" SCHED_NAME="default"
        N_TASKS=5
    
    elif [ "$DATASET" == "seq-cars196" ]; then
        MEM_SIZE=0 ONLINE_ITER=3 EVAL_PERIOD=100 F_EVAL_PERIOD=1000
        ...
        MEM_SIZE=0 ONLINE_ITER=3 EVAL_PERIOD=270 F_EVAL_PERIOD=2700
        BATCHSIZE=64; LR=1e-4 OPT_NAME="adam" SCHED_NAME="default"
        N_TASKS=5

    elif [ "$DATASET" == "seq-resisc45" ]; then
        MEM_SIZE=0 ONLINE_ITER=3 EVAL_PERIOD=500 F_EVAL_PERIOD=5000
        BATCHSIZE=64; LR=5e-4 OPT_NAME="adam" SCHED_NAME="default"
        N_TASKS=5
        ...
        MEM_SIZE=0 ONLINE_ITER=3 EVAL_PERIOD=100 F_EVAL_PERIOD=500
